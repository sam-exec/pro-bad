...
export function AdminSalesModule() {
  ...
  // Reports Sub-Tab: "employee" | "product" | "daily"
  const [reportType, setReportType] = useState<
    "employee" | "product" | "daily"
  >("employee");

  // Admin summary statistics
  const summary = useMemo(() => {
    return salesManagementService.getAdminSummary();
  }, [updateTrigger]);
  ...
  const handleExportExcel = () => {
    if (activeTab === "all-sales") {
      ...
      salesManagementService.exportToCSV("Admin_All_Shop_Records", headers, rows);
    } else {
      ...
      if (reportType === "employee") {
        ...
        salesManagementService.exportToCSV("Employee_Wise_Shop_Report", headers, rows);
        ...
        salesManagementService.exportToCSV("Product_Wise_Shop_Report", headers, rows);
        ...
        salesManagementService.exportToCSV("Daily_Shop_Report", headers, rows);
      }
    }
  };
  ...
  const handleExportPDF = () => {
    ...
    if (activeTab === "all-sales") {
      ...
      reportTitle = "Employee-Wise Shop Report";
      tableHtml = `
        <table>
          <thead>
            <tr>
      ...
      reportTitle = "Product-Wise Shop Report";
      tableHtml = `
        <table>
          <thead>
            <tr>
      ...
      reportTitle = "Daily Shop Records Report";
      tableHtml = `
        <table>
          <thead>
            <tr>
    ...
    salesManagementService.exportToPDF(
      reportTitle,
      "Pro Badminton Admin Reporting System • Facility Registry",
      tableHtml
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 border-b border-slate-200 pb-3">
        <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-xl border border-slate-200">
          ...
          >
            ...
            <span>All Transactions ({filteredSales.length})</span>
          </button>
      ...
      {activeTab === "reports" && (
        <div className="space-y-6">
          {/* Sub-Tabs: Employee-wise / Product-wise / Daily */}
          <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
            {[
              { id: "employee", label: "Employee-wise Performance" },
              { id: "product", label: "Product-wise Breakdown" },
          ...
          {/* Report 1: Employee-wise Performance */}
          {reportType === "employee" && (
            <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-slate-50/60 font-bold text-xs text-slate-800 uppercase tracking-wider flex items-center justify-between">
                <span>Employee-wise Performance</span>
          ...
          {/* Report 2: Product-wise Breakdown */}
          {reportType === "product" && (
            <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-slate-50/60 font-bold text-xs text-slate-800 uppercase tracking-wider flex items-center justify-between">
          ...
          {/* Report 3: Daily Transactions */}
          {reportType === "daily" && (
            <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-slate-50/60 font-bold text-xs text-slate-800 uppercase tracking-wider flex items-center justify-between">
                <span>Daily Transactions Timeline</span>
                <span className="text-[11px] text-slate-500 font-normal">
                  Aggregated by Date
                </span>
              </div>
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                    {dailyReportData.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="py-8 text-center text-slate-400">
                          No daily transactions data available.
                        </td>
                      </tr>
                    ) : (
