...
export function FlexibleMembershipModule() {
  ...
  useEffect(() => {
    const handleUpdate = () => setMemberships(membershipService.getSnapshotFlexible());
  ...
  }, [memberships, searchQuery, statusFilter, paymentMethodFilter]);
  ...
  const handleSave = async (
    recordData: Partial<FlexibleMembershipRecord>,
    idToEdit?: string
  ) => {
    if (idToEdit) {
      ...
      if (viewingRecord?.id === idToEdit) {
        setViewingRecord(updated);
      }
    } else {
      const join = recordData.joiningDate || new Date().toISOString().split("T")[0];
      const expiry = calculateExpiryDate(join, 45);
      const totalH = recordData.totalHours || 30;
      const usedH = recordData.hoursUsed || 0;
      ...
      const created = await membershipService.createFlexible(
        {
          primaryMemberName: recordData.primaryMemberName || "",
          primaryMobileNumber: recordData.primaryMobileNumber || "",
          ...
          planFee: recordData.planFee || 450,
          amountPaid: recordData.amountPaid || 450,
          dueAmount: recordData.dueAmount ?? Math.max(0, (recordData.planFee || 450) - (recordData.amountPaid || 450)),
          paymentMethod: recordData.paymentMethod || "Cash",
          paymentStatus: recordData.paymentStatus || "Paid",
          status: computedStatus,
          remarks: recordData.remarks,
          additionalMembers: recordData.additionalMembers || [],
        },
        {
          employeeId,
          employeeName,
...
export default FlexibleMembershipModule;
