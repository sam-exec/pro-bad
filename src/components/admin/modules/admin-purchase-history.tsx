...
export function AdminPurchaseHistory() {
  ...
  const [selectedBillForEdit, setSelectedBillForEdit] = useState<PurchaseBill | null>(null);
  const [selectedBillForPrint, setSelectedBillForPrint] = useState<PurchaseBill | null>(null);

  // Subscribe to changes in purchase history service
  useEffect(() => {
  ...
  // Handlers
  const handleSaveBill = (billData: any, idToEdit?: string) => {
    if (idToEdit) {
      purchaseHistoryService.updateBill(idToEdit, billData);
    } else {
      purchaseHistoryService.createBill(billData);
    }
  ...
  const handleDeleteBill = (id: string) => {
    if (
      window.confirm(
        "Are you sure you want to delete this purchase bill? The stock added from this bill will be automatically deducted from inventory."
      )
    ) {
      purchaseHistoryService.deleteBill(id);
      if (selectedBillForDrawer?.id === id) {
        setSelectedBillForDrawer(null);
      }
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Supplier tax invoices, procurement spend, GST input credit tracking, and stock intake.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          <ExportDropdown
            ...
            moduleTitle="Purchase History & Supplier Bills"
            subtitle="Facility Purchase Bills"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200/80 shadow-xs flex items-center justify-between">
          <div>
            ...
            <span className="text-xs text-slate-400 font-medium">Supplier bills logged</span>
          </div>
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200/80 shadow-xs flex items-center justify-between">
          <div>
            ...
            <span className="text-xs text-slate-400 font-medium">Stock added to inventory</span>
          </div>
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200/80 shadow-xs flex items-center justify-between">
          <div>
            ...
            <span className="text-xs text-slate-400 font-medium">Eligible input tax credit</span>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-3 items-stretch lg:items-center justify-between">
          <div className="flex-1 max-w-md">
            <UniversalSearch
              ...
              placeholder="Search invoice #, supplier name, GSTIN, item..."
              className="w-full"
            />
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
              ...
              {filteredBills.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-400">
                    ...
                    <p className="text-sm font-medium">No purchase bills match the filters.</p>
                  </td>
                </tr>
              ) : (
        ...
        <div className="px-4 py-3 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 flex justify-between items-center">
          <span>Showing {filteredBills.length} purchase vouchers</span>
          <span>Stock is automatically synced with Master Inventory</span>
        </div>
      </div>

      {/* Modals, Drawers & Print Previews */}
      <PurchaseBillModal
        isOpen={isModalOpen}
        onClose={() => {
          setIsModalOpen(false);
        ...
        billToEdit={selectedBillForEdit}
        isAdmin={true}
      />
      ...
      <PurchaseBillDrawer
        ...
        onDelete={(id) => handleDeleteBill(id)}
        onPrint={(bill) => {
